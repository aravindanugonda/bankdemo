...
\c "postgres";
CREATE DATABASE "BANK_PAC" ENCODING UTF8;
...
-- *    Copyright (C) 1984-2025 Rocket Software, Inc. or its affiliates. All Rights Reserved.
-- *
-- *    $Id: DeqRequestNativeV3.sql 1089712 2021-09-10 14:30:23Z cpj $
-- **************************************************************************
...
--               return code defined by IBM for this case. An abend would occur on the mainframe
--
CREATE OR REPLACE FUNCTION DBRG_BANKPAC."fn_deq_request_native_v3"(p_requestType INT,         --  Type of DEQ request
                                                             p_qnameKey BYTEA,          --  Major name (used as part of the primary key)
                                                             p_rnameKey BYTEA,          --  Minor name (used as part of the primary key)
                                                             p_scope VARCHAR(8),        --  Scope of request (used as part of the primary key)
                                                             p_taskNumber BIGINT,       --  Task making the request (used as part of the primary key)
...
BEGIN
    ...
    --  Check if the task has already issued a request for the resource
    --
    IF p_requestType = 5 --FORCEDEQ
    THEN
        IF p_processRecordId = -1 --don't match against process record ID
        THEN
            SELECT Id, Owned
              FROM DBRG_BANKPAC."$$ACTIVE$$Resource_Locks"
             WHERE Owned = 'true' AND
                   QNameKey::bytea || REPEAT(' ', 8-length(QNameKey))::bytea = p_qnameKey || REPEAT(' ', 8-length(p_qnameKey))::bytea AND
                   RNameKey::bytea || REPEAT(' ', 256-length(RNameKey))::bytea = p_rnameKey || REPEAT(' ', 256-length(p_rnameKey))::bytea AND
                   Scope = p_scope AND
                   Region = p_region
             INTO l_id, l_owned;
        ELSE
